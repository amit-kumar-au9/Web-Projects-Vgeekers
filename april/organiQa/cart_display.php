...
<head>
    ...
    <link href="https://fonts.googleapis.com/css?family=Cookie|Dancing+Script|Marck+Script|Zhi+Mang+Xing&display=swap" rel="stylesheet">
    ...
    <link rel="stylesheet" type="text/css" href="asset/css/empty_cart.css">
...
<body>
    <?php
        ...
        $user_id=$results['id'];
        $fna=$results['first_name'];
        $lna=$results['last_name'];
        $ph=$results['phone'];
        $email=$results['email'];
    ...
    ?>  
    <center>
      <button class="btn btn-outline-success" id="cc">Total items in cart= <?php echo $rowss?></button>
      <a href="form.php" target="_parent" style="text-decoration: none">
        <button class="btn btn-outline-danger" id="bb" style="display: none">Kindly Login/Sign Up first.</button>
      </a>
      <?php
        if(isset($_SESSION['id'])){
           echo"<script>document.getElementById('cc').style.display='block';
                document.getElementById('bb').style.display='none';</script>";           
        }
        else{
          echo "<script>document.getElementById('bb').style.display='block';
                document.getElementById('cc').style.display='none'</script>";
        }
      ?>
      <div class="empty-cart" id="ok" style="display: none">
      ...
      <a href='checkout.php' target="_parent">
        <button class='btn btn-outline-primary' type='submit' id="aaa">Proceed For Checkout</button>
      </a>
          ...
          <?php
            if($result->num_rows > 0){
              while($row=$result->fetch_assoc()){  
                echo"   <tr>
                            <td><img src='asset/images/products/".$row['image']."' width='100' height='100'></td>
                            <td>".$row["Product_name"]."(".$row["quan"].")<br><br>Rs".$row["pro_cost"]."/-</td>
                            <td>
                                <form action='add_cart.php' method='post'>
                                    <input type='text' value='".$row['time']."' name='time' style='display:none'>
                                    <input type='text' value='".$row['product_id']."' name='name' style='display:none'>
                                    <input type='text' value='".$row['Product_price']."' name='price' style='display:none'>
                                    <button class='btn btn-outline-success' type='submit'><i class='fa fa-plus'></i></button>
                                </form>
                                <form action='delete_cart.php' method='post'>
                                    <input type='text' value='".$row['time']."' name='time' style='display:none'>
                                    <input type='text' value='".$row['product_id']."' name='name' style='display:none'>
                                    <input type='text' value='".$row['Product_price']."' name='price' style='display:none'>
                                    <button class='btn btn-outline-danger' type='submit'><i class='fa fa-minus'></i></button>
                                </form>
                            </td>
                        </tr>";                   
              }    
                ...
                echo"<br><b>Your cart is empty.Kindly check my orders or place new orders.</b>";
                echo"
                    <script>
...
<script type="text/javascript">
    localStorage.setItem("lastname", <?php echo $rowss?>);
