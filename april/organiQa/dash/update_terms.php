<?php include "include/header.php";?>
	<div id="page-wrapper" style="background: white">
		<div class="main-page" style="background: white;">
		    <?php 
		        include 'conn.php';
		        $checkPrivacy = "SELECT * FROM detail WHERE id = 3";
		        $checkResult = mysqli_query($con, $checkPrivacy);
		        $row = $checkResult -> fetch_assoc();
		    ?>
			<center><h2 class="title1" style="color: red;">Terms and Conditions</h2></center>
			<div class="forms">
...
<?php 
  ...
  if(isset($_POST['addTestimonial'])){ //check if form was submitted
        ...
        if ($content !="") {
            $checkQuery = " UPDATE detail SET content = '".$content."' WHERE id = 3 ";
    
            if (mysqli_query($con, $checkQuery)) {
                echo "<script>window.alert('Terms & Condition Updates');window.location.href='update_terms.php';</script>";
            }
            else{
                echo "<script>window.alert('Please Retry');window.location.href='update_terms.php';</script>";
              }
        }
