...
<html lang="en">
  ...
  <body class="goto-here">
   ...
   <div class="hero-wrap hero-bread" style="background-image: url('https://images.unsplash.com/photo-1582615908486-aa0a3958e60e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1951&q=80');">
      <div class="container">
    ...
    <?php
      $sql="SELECT orders.total,orders.status,product_display.Product_name, product_display.image, product_display.Product_price,orders.time,orders.id,signup.address,order_product.order_id FROM orders, product_display, signup, order_product WHERE  order_product.product_id= product_display.id and orders.user_id='$user_id' and signup.id='$user_id' and orders.id= order_product.order_id GROUP BY order_product.order_id";
        $result=$con->query($sql);
        $rowss=$result->num_rows;
        $query30="SELECT product_display.Product_name,order_product.product_id FROM product_display,order_product WHERE order_product.product_id=product_display.id";
        $data30= mysqli_query($con,$query30);
        if($data30->num_rows > 0){
          while($row20=$data30->fetch_assoc()){
              $a=$row20['Product_name'];
          }
        }
                  ...
                  if($result->num_rows > 0){
                    while($row=$result->fetch_assoc()){
                      if($row["status"]!="cancled"){
                        ...
                        echo"<center><tr><td>".$row['order_id']."</td><td>".$a."</td><td>Rs".$row["total"] . "/-</td></center><td><button class='btn btn-outline-success' type='submit'>Yet to be delivered</button><form action='delete_orders.php' method='post'><input type='text' value='".$row['time']."' name='time' style='display:none'><button class='btn btn-outline-danger' type='submit'>Cancle order</button></form></td><td>".$row["address"]."</td></tr>";
                      }
                      else{
                        echo"<center><tr><td>".$row['Product_name']."</td><td>Rs".$row["total"] ."/-</td></center><td><button class='btn btn-outline-danger' type='submit'>Cancled</button></td><td>".$row["address"]."</td></tr>";
                      }                     
                    }
                  }
                  else{
