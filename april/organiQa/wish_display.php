...
<body>
  <?php
    ...
    $ph=$results['phone'];
    $email=$results['email'];
    $sql="SELECT product_display.Product_name, product_display.image, wishlist.product_id, product_display.Product_price,wishlist.time FROM wishlist, product_display WHERE wishlist.product_id= product_display.id and wishlist.user_id='$user_id'";
    $result=$con->query($sql);
    $rowss=$result->num_rows;
  ?> 
  <center>
    <button class="btn btn-outline-success" id="cc">Total items in wishlist= <?php echo $rowss?></button>
          ...
          if($result->num_rows > 0){
            while($row=$result->fetch_assoc()){  
              ...
              echo" <tr>
                      ...
                      <td>".$row["Product_name"]."<br><br>Rs".$row["Product_price"]."/-</td>
                      <td>
                        <form action='delete_wish.php' method='post'>
                          <input type='text' value='".$row['time']."' name='time' style='display:none'>
                          <button class='btn btn-outline-danger' type='submit'><i class='fa fa-times'></i></button></form>
                      </td>
                    </tr>";     
            }        
          }
            ...
            echo "<br><b>Your wishlist is empty</b>";
            echo"<script>
