...
<div class="col-md-6" style="display: inline-block;">
	<div class="card" >
  		<div class="card-body animate__animated animate__zoomIn">
    		<h2 class="card-title p"><span style="color: red">S</span>wastick <span style="color: red">C</span>ommon<span style="color: red"> A</span>pptitude<span style="color: red"> T</span>est <br><span style="color: red" class="a">"</span><span class="a"> 1.0 </span> <span style="color: red" class="a">"</span></h2>
    		<h4 class="card-text">Get andmission in the most prestigious and trusted  IIT JEE Coaching Institute</h4>
    		<p class="card-text"><span style="color: red;font-family: arial">EXAMS DATES: <span style="color: white;"> 23 MAY 2020</span></span></p>
  		</div>
	</div>
	<div class="col-md-2">
		<div class="student">
		...
		<animate xlink:href="#pgFld" attributeName="d" begin="4s" dur="5s" repeatCount="indefinite" keyTimes="0;
		                         .03;
		                         .06;
		                         .1;
		                         .15;
...
<div class="col-md-6" style="float: right;margin-top: 250px">
	<div class="loginForm">
    	<h2 style="margin: 10px;font-size: 35px" align="center">Registration</h2>
        <div class="formBody">
            <form class="loginForm">
	            <div>
	                ...
	                <div class="formInput form-group">
	                    <label for="student_class"><span class="red">*</span> Student's Class</label>
	                ...
	                <div class="formInput">
	                    <div>Phone</div>
	                    <input type="number" maxlength="10" name="mobile" placeholder="enter phone">
	                </div>
	                <div class="formInput">
	                    <a class="btn btn-outline-success send-otp">send otp</a>
	                	<div class="e-msg-otp"></div>
	                </div>
	                <div class="formInput" id="otp-field">
	                    <div>Enter OTP</div>
	                    <input type="number" maxlength="10" name="otp" placeholder="enter otp">
	                </div>
	            </div>
	            <button type="submit" class="btn btn-outline-danger submit-btn" style="width: 60%">Login</button>
	            <div class="e-msg-submit"></div>
		        <p align="center" id="rzp-button1">
		            <br><br>
		            <button type="submit" class="btn btn-primary form-control checkout" style="background: #ddb42c; font-size: 18px; color: #000; font-weight: 700; font-size: 18px;">PROCESS CHECKOUT</button>
		        </p>
	        </form>
...
<script id="rendered-js">
	...
	<script type="text/javascript">
		...
		$('.send-otp').on('click', function(){
			...
			if(mobile != '' && mobile != NaN && mobile != null && mobile != undefined){
				$.ajax({
					'type' : 'post',
					'url' : 'send-otp.php',
					'data' : 'mobile='+mobile,
					success : function(response){
						...
						if(!json.error){
							$('div.e-msg-otp').addClass('success').removeClass('error').text(json.message);
							$('.submit-btn').show();
							$('#otp-field').show();
						}
						else{
							$('div.e-msg-otp').addClass('error').removeClass('success').text(json.message);
							$('.submit-btn').hide();
							$('#otp-field').hide();
						}
					}
				...
				$('div.e-msg-otp').addClass('error').removeClass('success').text('Please Enter Mobile Number');
			}
		});
	...
	<script type="text/javascript">
		$('.checkout').click(function(e){
	    	...
	    	var options = {
			    "key": "rzp_live_jDqNtTpm9nJasv", // Enter the Key ID generated from the Dashboard
			    "amount": "100", // Amount is in currency subunits. Default currency is INR. Hence, 50000 means 50000 paise or ₹500.
			    "currency": "INR",
			    "name": "Swastik",
			    "description": "Swastik",
			    "image": "",
			    // "order_id": "order_9A33XWu170gUtm",// Replace this with the order_id created using Orders API (https://razorpay.com/docs/api/orders).
			    "handler": function (response){
			        // alert(response.razorpay_payment_id);
			    	$.ajax({
			    		'type' : 'post',
			    		'data' : formData+'&payment_id='+response.razorpay_payment_id,
			    		'url' : 'insert-checkout.php',
			    		success : function(response){
			    			var json = JSON.parse(response);
			    			if(!json.error){
			    				setTimeout(function(){
						        	window.location.href='phase3.php?token='+btoa(formData);
						        }, 500);
		    				}
			    			else{
			    				//
