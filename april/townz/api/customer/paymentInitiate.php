<?php 
	...
	$checkQuery = "SELECT * FROM customer_details WHERE id = '".$userId."'";
	...
	if(mysqli_num_rows(mysqli_query($conn, $checkQuery)) >0){
     ...
     mysqli_close($conn);
	    ...
	    $date = date('Y/m/d');
		$insertOrder = "INSERT INTO order_txn_customer(txn_id, wallet_id, amount, name, currency, status, contact_no, email_id, order_id, description,date, type) VALUES ('".$last_id."', '".$walletId."', '".$amount."', '".$name."', '".$currency."', '".$payStatus."', '".$userContact."', '".$userEmail."', '".$orderId."', '".$description."', '".$date."', 'Recharge')";
