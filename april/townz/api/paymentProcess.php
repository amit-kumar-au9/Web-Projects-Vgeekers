...
// 	$desc = JSON_encode($decoded['pay_desc']);
	...
	if(isset($amount) && isset($balance)){
	    ...
	    $checkQuery = "SELECT * FROM transaction_table WHERE id = '".$txn_id."'";
        ...
        //     $conn = new mysqli($servername, $username, $password, $database);
    		...
    		$time = date("h:i:s A");
    		$updateTxn = "UPDATE transaction_table SET payment_status = '".$payment_status."', dsc = '".$desc."', gateway_order_id = '".$gateway_order_id."', pay_error_code = '".$pay_error_code."', pay_id = '".$pay_id."', pay_captured = '".$pay_captured."', time = '".$time."', pay_signature = '".$pay_signature."' WHERE id = '".$txn_id."' ";
    		mysqli_query($conn, $updateTxn);
    		date_default_timezone_set('Asia/Kolkata');
    		$time = date("h:i:s A");
    		$updateOrder = "UPDATE order_txn SET status = '".$payment_status."', time = '".$time."' WHERE txn_id = '".$txn_id."'";
    		mysqli_query($conn, $updateOrder);
    		
    		if($pay_error_code === 200){
    		    $balance = $balance + $amount;
    		    $updateWallet = "UPDATE wallet SET total_balance = '".$balance."' WHERE wallet_id = '".$wallet_id."'";
    		    mysqli_query($conn, $updateWallet);
    		    $updateVendor = "UPDATE vendor_details SET active = '1' WHERE user_id = '".$userId."'"; 
