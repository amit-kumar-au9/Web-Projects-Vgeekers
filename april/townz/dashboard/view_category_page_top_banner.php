	...
	<div class="main-page">
		...
		<table class="responstable" >
			...
			<?php 
				...
				$checkCategoryImage = "SELECT * FROM customer_app_banner WHERE location = 'category_top'";
	...
	if(isset($_POST['update'])){ //check if form was submitted
	   ...
	   if ($file_upload) {
	       ...
	       if (mysqli_query($conn, $checkQuery)) {
	           ...
	           unlink($image);
	           echo "<script>window.location.href='view_category_page_top_banner.php';</script>";
	       }
	       else{
	           echo "<script>window.alert('Please Re Upload Image');window.location.href='view_category_page_top_banner.php';</script>";
	       }
	   }
	   else{
	       echo "<script>window.location.href='view_category_page_top_banner.php';</script>";
	 ...
	 if(isset($_POST['add'])){ //check if form was submitted
	   ...
	   if ($file_upload) {
	       $checkQuery = "INSERT INTO customer_app_banner (image, location) VALUES('".$file_name."','category_top')";
	       if (mysqli_query($conn, $checkQuery)) {
	           echo "<script>window.location.href='view_category_page_top_banner.php';</script>";
	       }
	       else{
