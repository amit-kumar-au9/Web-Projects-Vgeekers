...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function allReports(){
		...
		//expenses
		$data['todayExpenses'] = $this->db->select('sum(amount) as item')->where('date',date("Y-m-d"))->where('delete_status',0)->get('expense')->result();
		$data['weekExpenses'] = $this->db->select('sum(amount) as item')->where('date BETWEEN "'.$from.'" AND "'.$to.'"')->where('delete_status',0)->get('expense')->result();
		$data['monthExpenses'] = $this->db->select('sum(amount) as item')->like('date',date('Y-m'))->where('delete_status',0)->get('expense')->result();
		$data['yearExpenses'] = $this->db->select('sum(amount) as item')->like('date',date('Y'))->where('delete_status',0)->get('expense')->result();
		$data['allExpenses'] = $this->db->select('sum(amount) as item')->where('delete_status',0)->get('expense')->result();
