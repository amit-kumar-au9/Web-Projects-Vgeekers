...
// 						 ->result();
        return $this->db->query('
            SELECT pr.product_id, pr.code, pr.name, sum(pi.quantity) as pquantity, sum(pi.quantity*pi.cost) as pptotal, (select sum(si.quantity*si.price) as sptotal from sales_items si where si.product_id = pr.product_id) as sptotal, (select sum(quantity) as squantity from sales_items si where si.product_id = pr.product_id) as squantity, (select sum(si.quantity*si.price) - sum(si.quantity*pi.cost) from sales_items si where si.product_id = pr.product_id) as profit FROM products pr INNER JOIN purchase_items pi ON pi.product_id = pr.product_id LEFT JOIN purchases p ON p.purchase_id = pi.purchase_id GROUP BY pr.product_id
	...
	*/
	public function getProductsDetails($product_id,$warehouse_id,$start_date,$end_date){
			return $this->db->query('
											...
											SELECT pr.product_id, pr.code, pr.name, sum(pi.quantity) as pquantity, sum(pi.quantity*pi.cost) as pptotal, (select sum(si.quantity*si.price) as sptotal from sales_items si where si.product_id = pr.product_id) as sptotal, (select sum(quantity) as squantity from sales_items si where si.product_id = pr.product_id) as squantity, (select sum(si.quantity*si.price) - sum(si.quantity*pi.cost) from sales_items si where si.product_id = pr.product_id) as profit FROM products pr INNER JOIN purchase_items pi ON pi.product_id = pr.product_id LEFT JOIN purchases p ON p.purchase_id = pi.purchase_id LEFT JOIN warehouse w ON w.warehouse_id = p.warehouse_id 
											WHERE 
												(pr.date >= ? OR ? IN ("",NULL))
											AND
												(pr.date <= ? OR ? IN ("",NULL))
	...
	*/
	public function getProductsDetailsForCSV($product_id,$start_date,$end_date){
				return $this->db->query('
											SELECT pr.product_id, pr.code as "Code", pr.name as "Product Name", sum(pi.quantity) as "Purchase Quantity", sum(pi.quantity*pi.cost) as pptotal, (select sum(si.quantity*si.price) as "Total" from sales_items si where si.product_id = pr.product_id) as "Total", (select sum(quantity) as "Sales Quantity" from sales_items si where si.product_id = pr.product_id) as "Sales Quantity", (select sum(si.quantity*si.price) - sum(si.quantity*pi.cost) from sales_items si where si.product_id = pr.product_id) as "Profit" FROM products pr INNER JOIN purchase_items pi ON pi.product_id = pr.product_id LEFT JOIN purchases p ON p.purchase_id = pi.purchase_id LEFT JOIN warehouse w ON w.warehouse_id = p.warehouse_id 
