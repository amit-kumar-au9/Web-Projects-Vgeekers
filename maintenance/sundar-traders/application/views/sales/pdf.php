	...
	<table width="670px" border="1" cellspacing="0" style="border: 0px solid black; border-collapse: collapse;" class="table" cellpadding="2">
		...
		<tr>
			<td valign="top" rowspan="" style="text-align:left;font-size: 13px;padding: 10px;" colspan="5">
					...
					<span style="font-size: 12px;"><b><?php if(isset($data[0]->customer_name)){echo $data[0]->customer_name; }?><br/></b></span>
			...
			<td colspan="5" valign="top" style="padding: 10px;">
				...
				<span style="font-size: 12px;"><b><?php if(isset($data[0]->customer_name)){echo $data[0]->customer_name; }?></b></span><br/>
		...
		<tr>
			...
			<tr>
				...
				<?php 
					if($value->tax_type =="Inclusive"){
				?>
				<td align="center" class="fontS"><?php echo $value->gross_total - $value->igst_tax -$value->cgst_tax -$value->sgst_tax;?></td>
				<?php 
					}else{
				?>
				<td align="center" class="fontS"><?php echo $value->gross_total;?></td>
				<?php
					}
				?>
				<td align="right" class="fontS" colspan="2"><?php echo $value->discount."(".$value->discount_value."%)";?></td>
				...
				<td align="center" class="fontS" colspan="2">
					<?php 
						if($value->tax_type =="Inclusive"){
							...
							echo ($value->gross_total - $value->discount) ;//1800
						}	
					?>
				</td>
			...
			<?php 
					$i++;
					if($value->tax_type =="Inclusive"){
						$tot += $value->gross_total - $value->discount - $value->igst_tax - $value->cgst_tax - $value->sgst_tax;
						$totD += $value->gross_total - $value->igst_tax - $value->cgst_tax - $value->sgst_tax;
					}else{
						$tot += $value->gross_total - $value->discount;
						$totD += $value->gross_total;
					}
        ...
        <tr>
        	...
        	<td colspan="14" valign="top" style="height: 60px;border-bottom: 0px;font-size: 13px;">
        		...
        		<b> INR <?php echo $this->numbertowords->convert_number(round($data[0]->total+$data[0]->shipping_charge-$data[0]->flat_discount)); ?> Only</b>
        	</td>
        </tr>
        <tr>
